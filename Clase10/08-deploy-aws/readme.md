...
```
eksctl create cluster --name cdocker18 --without-nodegroup --region us-east-1 --zones us-east-1a,us-east-1b,us-east-1c --version 1.32
```

### Crear nodos

```
eksctl create nodegroup --cluster cdocker18 --name cdocker18-nodegroup --node-type t3.medium --nodes 1 --nodes-min 1 --nodes-max 3 --asg-access
```

### Crear IAM Provider

```
eksctl utils associate-iam-oidc-provider --cluster cdocker18 --approve
```
...
```
aws iam create-policy --policy-name AWSLoadBalancerPolicy_cdocker18 --policy-document file://iam_policy.json
```

### Crear la cuenta de servicio para el cluster

```
eksctl create iamserviceaccount --cluster cdocker18 --namespace=kube-system --name=aws-lb-ms-cdocker18 --attach-policy-arn=arn:aws:iam::282865065290:policy/AWSLoadBalancerPolicy_cdocker18 --override-existing-serviceaccounts --approve
```
...
```
helm upgrade -i aws-load-balancer-controller eks/aws-load-balancer-controller --set clusterName=cdocker18 --set serviceAccount.create=false --set serviceAccount.name=aws-lb-ms-cdocker18  -n kube-system
```
...
```
aws eks update-kubeconfig --name clusterdocker17 --region us-east-1
```
